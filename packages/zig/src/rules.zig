...
fn tailwindGroupIndex(cls: []const u8) u8 {
    // Strip variant prefixes like hover:, md:, dark:, focus:, etc.
    var base = cls;
    while (true) {
        const colon = std.mem.indexOfScalar(u8, base, ':') orelse break;
        base = base[colon + 1 ..];
    }
...
fn tailwindVariantPriority(cls: []const u8) u8 {
    ...
    // Has any variant prefix (hover:, focus:, dark:, etc.)
    if (std.mem.indexOfScalar(u8, cls, ':') != null) return 20;
    return 0;
}
